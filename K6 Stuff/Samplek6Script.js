...
// Before running a test do calls like login
export function setup() {
    const headers = { "Content-Type": "application/json", "Authorization": "", "site-type": "J"}
    // Login for the user
    const url = "";
    ...
    const sig = loginBody.responseObject.CDAPHeader.Sig
    const ses = loginBody.responseObject.CDAPHeader.Ses

    return {ses: ses, sig: sig}
}

// Main function
export default function (data) {

    const headers = { "Content-Type": "application/json", "Authorization": data.ses, "site-type": ""}
...
//After test completion do logout
export function teardown(data) {
    // Logout for the user
    const headers = { "Content-Type": "application/json", "Authorization": "Bearer " + data.token, "site-type": "J"}

    const logoutUrl = ""

    let logoutBody = ""

    let res = http.post(logoutUrl, logoutBody, {headers: headers});

    console.log("Logout: " + (res.status === 200 ? 'success' : 'failed'))
}